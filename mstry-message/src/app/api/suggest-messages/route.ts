...
export async function POST(req: Request) {
    try {
        const prompt = "Create a list of three open-ended and engaging questions formatted as a single string. " +
            ...
            "your output should be structured like this: 'What's a hobby you've recently started? " +
            "||If you could have dinner with any historical figure, who would it be? || What's a simple thing that makes you happy?'." +
            " Ensure the questions are intriguing, foster curiosity, " +
            "andcontribute to a positive and welcoming conversational environment."
        ...
        if (error instanceof APICallError) {
            const {name, statusCode, responseHeaders, responseBody} = error;

            return NextResponse.json({name, statusCode, responseHeaders, responseBody}, {status: statusCode})
        } else {
