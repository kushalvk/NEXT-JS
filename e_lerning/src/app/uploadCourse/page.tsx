...
interface UploadedCoursesResponse {
    success: boolean;
...
const EmptyState = ({ query }: { query: string }) => (
    ...
    >
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-10 max-w-md mx-auto border border-white/20">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">No courses uploaded</h3>
            <p className="text-gray-300 mb-6">
                {query
                    ? `No courses match "${query}"`
                    : 'You haven\'t uploaded any courses yet. Start sharing your knowledge!'}
            </p>
...
const UploadCoursePage: React.FC = () => {
    ...
    // ----------------------------------------------------------------
    // Fetch user + uploaded courses
    // ----------------------------------------------------------------
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [userRes, courseRes] = await Promise.all([loggedUser(), userUploadedCourse()]);
            ...
            // Uploaded Courses
            if ((courseRes as UploadedCoursesResponse)?.success) {
                setUploadedCourses((courseRes as UploadedCoursesResponse).Course);
            }
        } catch (err) {
            ...
            <div className="min-h-screen flex flex-col items-center p-4 sm:p-6 lg:p-8 pt-20 lg:pt-24 font-sans mt-21">
                ...
                >
                    ...
                    <p className="text-gray-300 text-lg">
                        Manage and promote the courses you’ve created.
                    </p>
                </motion.div>
                ...
                >
                    <Button
                        asChild
                        className="bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-600 hover:to-purple-700 text-white font-bold px-8 py-4 rounded-xl text-lg shadow-xl transform transition-all duration-300 hover:scale-105"
                    >
                        <Link href="/addCourse">Add New Course</Link>
                    </Button>
                </motion.div>
                ...
                >
                    ...
                    <p className="text-gray-300 text-lg mb-8 max-w-2xl mx-auto">
                        Create courses that inspire and help learners grow.
                    </p>
                    <Button
                        asChild
